...
// TOKEN_HEADER use to save keystone token
...
func (c *Client) GetToken(auth Auth) (string, error) {
	...
	resp, err := c.doRequest(request{
		URL:          fmt.Sprintf("%s/auth/tokens", c.URL),
	...
	token := resp.Headers.Get(TOKEN_HEADER)
	if token == "" {
